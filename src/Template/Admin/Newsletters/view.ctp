...
<div class="newsletters view large-10 medium-9 columns">
    <h2><?= h($newsletter->name) ?></h2>
    <div class="row">
        <div class="large-5 columns strings">
            <h6 class="subheader"><?= __('Name') ?></h6>
            <p><?= h($newsletter->name) ?></p>
        </div>
        <div class="large-2 columns numbers end">
            <h6 class="subheader"><?= __('Id') ?></h6>
            <p><?= $this->Number->format($newsletter->id) ?></p>
            <h6 class="subheader"><?= __('Status') ?></h6>
            <p><?= $this->Number->format($newsletter->status) ?></p>
            <h6 class="subheader"><?= __('Sent Count') ?></h6>
            <p><?= $this->Number->format($newsletter->sent_count) ?></p>
        </div>
        <div class="large-2 columns dates end">
            <h6 class="subheader"><?= __('Created') ?></h6>
            <p><?= h($newsletter->created) ?></p>
            <h6 class="subheader"><?= __('Modified') ?></h6>
            <p><?= h($newsletter->modified) ?></p>
        </div>
    </div>
    <div class="row texts">
        <div class="columns large-9">
            <h6 class="subheader"><?= __('Content') ?></h6>
            <?= $this->Text->autoParagraph(h($newsletter->content)) ?>
        </div>
    </div>
</div>
